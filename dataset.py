...
import ds_utils.neuroseg_2020 as utils
...
class RobotSegDataset(Dataset):
    ...
    def get_filename_from_idx(self, idx):
        ...
        if 'TAPNet' in self.model and self.mode == 'train':
            if self.schedule == "shuffle":
                ...
                print("Index-> ", idx)
                print("Length of ordered filenames-> ", len(self.ordered_filenames))
    ...
    def load_optflow(self, filename):
        # read .flo file
        #for jpg check jpg 
        with open(str(filename).replace('images', 'optflows').replace('jpg', 'flo')) as f:
